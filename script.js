...
// Main Live Story Cinematic Loop
async function initiateLoveStoryTimeline() {
    // --- SCENE 1: Starry Dark Intro ---
    document.getElementById('envStars').style.opacity = '0.5';
    document.getElementById('envMoon').style.opacity = '0.75';
    ...
    // --- SCENE 2: Broken Heart & Downpour ---
    document.getElementById('envRain').style.opacity = '0.6';
    mainViewport.style.background = '#0b0914';
    allocateParticles('💔', 8);
    ...
    mainViewport.style.background = '#261733'; 
    ...
    // --- SCENE 4: Explicit Heartfelt Commitments ---
    ...
    mainViewport.style.background = '#1a102f'; 
    allocateParticles('🦋', 15);
    ...
    // Brighten Moon
    document.getElementById('moonNode').style.boxShadow = '0 0 85px #ffffff';
    allocateParticles('❤️', 30);
    await hold(5000); 

    // Fade UI display box back in
    ...
    textRenderer.innerHTML = `<h3>🌸</h3><h2 style='margin-top:10px;'>Shall We Start Chapter 2 Together?</h2>`;
    controlsTray.classList.add('active-tray');
}

// Choice Button Processing Entrypoint
function processPathSelection(selectedArgument) {
    ...
    if (selectedArgument === 'yes') {
        ...
        allocateParticles('❤️', 50);
        allocateParticles('✨', 40); 
        document.getElementById('moonNode').style.transform = 'translateX(-50%) scale(1.3)';
        
        textRenderer.innerHTML = `
            <h2 style="color: var(--love-pink); font-size: 2.2rem; text-shadow: 0 0 15px rgba(255,77,109,0.5);">I LOVE YOU SUCHI MA</h2>
        ...
        textRenderer.innerHTML = `
            ...
            <p style="color: var(--love-pink); margin-top:20px; font-weight:700;">❤️ I'll Never Stop Choosing You.</p>
        `;
    }
}
...
// Trigger setup execution hook on load
